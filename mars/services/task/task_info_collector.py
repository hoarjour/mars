...
class TaskInfoCollector:
    ...
    async def collect_subtask_operand_structure(self,
        ...
        for subtask in subtask_graph.topological_iter():
            ...
            await self.save_task_info({subtask.subtask_id: subtask_dict}, subtask_save_path, session_id)
    ...
    async def collect_last_node_info(self, task: Task, subtask_graphs: List[SubtaskGraph]):
        ...
        await self.save_task_info({"op": last_op_keys,
    ...
    async def collect_runtime_subtask_info(self,
        ...
        save_path = os.path.join(subtask.session_id, subtask.task_id, "subtask_runtime.yaml")
        await self.save_task_info({subtask.subtask_id: subtask_dict}, save_path, subtask.session_id)
    ...
    async def collect_runtime_operand_info(self,
        ...
        save_path = os.path.join(subtask.session_id, subtask.task_id, "operand_runtime.yaml")
    ...
    async def collect_fetch_time(self,
        ...
        await self.save_task_info({subtask.subtask_id: {"fetch_time": {"start_time": fetch_start,
                                                                       "end_time": fetch_end}}},
                                  save_path,
                                  subtask.session_id)
